...
def date_converter(switch, date_str):
    start = date_str.find('/')
    month = date_str[:start]
    second = date_str.find('/', start+1)
    day = date_str[start+1:second]
    year = date_str[second+1:]
    return day + " " + switch[int(month)] + " " + year
